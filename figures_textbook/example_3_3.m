...
x_init(1) = x_init(1) + pi/6;                                   %bus1の周波数偏差の初期値を(平衡点)+pi/6にセット
x_init(3) = x_init(3) + 0.1;                                    %bus1の内部電圧の初期値を(平衡点)+0.1にセット
out = net.simulate([0, 50], 'x0_sys', x_init);                  %シミュレーション実行(0から50秒まで 初期値=x_init)
plot(out.t, out.X{1}(:, 2), '-b', out.t, out.X{3}(:, 2), '--r') %bus1,3の周波数偏差の時間応答をプロット
ylim([-0.015, 0.015])
